...
public class TestProtobufStreamRDF {
    ...
    private void testGraph(boolean delimited) {
        ...
        if ( delimited ) {
            StreamRDF stream = PBinRDF.streamToOutputStream(out);// , true) ; // With values.
            StreamRDFOps.graphToStream(graph, stream) ;
        } else {
            PBinRDF.streamToOutputStreamBlk(out, s -> StreamRDFOps.graphToStream(graph, s));// , true) ; // With values.
        }
      ...
      // Delimited
      if ( delimited )
          PBinRDF.inputStreamToStreamRDF(in, stream2) ;
      else
          PBinRDF.inputStreamBlkToStreamRDF(in, stream2) ;
